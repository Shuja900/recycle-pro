...
include('Db.php');
$select = "SELECT *,p.id as pid FROM wr_product p Left Join wr_product_price r on p.id=r.product_id left join wr_model m on r.variant=m.id left join wr_brands b on p.brand=b.id where p.category=2 OR p.category=7 AND p.status='Show'";
$recordyr = mysqli_query($con,$select);

if($recordyr->num_rows > 0){
    $separator = ",";
    $filename = "selltabmyphone.csv";
