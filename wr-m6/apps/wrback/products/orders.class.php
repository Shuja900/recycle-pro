...
<strong>Date: </strong><?php echo $row['top']; ?><br />
										...
										<td>
										<?php
									$sql3 = "select * from ".WR_ORDER_DETAIL." where order_id='".$row['id']."' order by id desc";
									$rec = $this->db->fetch_query($sql3,$this->db->pdo_open());
									foreach ($rec as $row3)
									{
									    ?>
...
?>
                                    ...
                                    <div class="col-sm-4 invoice-col">
                                       ...
                                       <br>
                                        <b>Enter Note :</b><form method="POST" action="addnte.php"><input type="hidden" name="odi" id="odi" value="<?php echo $row['id']; ?>" /><input type="hidden" name="uname" id="uname" value="<?php echo $row2['fname'].' '.$row2['lname']; ?>"><textarea name="review" rows="4" cols="50"></textarea></br><button style="margin-top:2%;" type="submit" class="btn btn-primary"> ADD</button></form>
                                    </div>
                                    <!-- /.col -->
                                 </div>
                                 <!-- /.row -->
                                       ...
                                       <table class="table table-striped">
                                          ...
                                          <tbody>
											...
											foreach ($record_o as $row_o)
											{
												...
												$sqlb_p = "select * from ".wr_mobile_networks." where id='".$row_o['network']."' ";
												$rowy_p = $this->db->row($sqlb_p,$this->db->pdo_open());
                                    ...
                                    <div class="col-xs-3">
                                       <p class="lead">Order Status:</p>
                                        <p class="text-muted well well-sm no-shadow" style="margin-top: 10px; font-size:18px;">
									   	  <input type="hidden" name="uzr_id" id="uzr_id" value="<?php echo $row['user_id']; ?>" />	
                                          <input type="hidden" name="order_id" id="order_id" value="<?php echo $row['id']; ?>" />
                                          <input type="hidden" name="prc" id="prc" value="<?php echo $row['g_total']; ?> " />
...
<div style="color:#ffffff;font-family:Lato, Tahoma, Verdana, Segoe, sans-serif;line-height:1.2;padding-top:30px;padding-right:10px;padding-bottom:0px;padding-left:10px;">
<div style="font-size: 14px; line-height: 1.2; color: #ffffff; font-family: Lato, Tahoma, Verdana, Segoe, sans-serif; mso-line-height-alt: 17px;">
<p style="font-size: 34px; line-height: 1.2; word-break: break-word; text-align: center; mso-line-height-alt: 41px; margin: 0;"><span style="font-size: 34px;"><strong>Dear '.$row_usr['fname'].' '.$row_usr['lname'].',</strong></span></p>
</div>
</div>
<!--[if mso]></td></tr></table><![endif]-->
...
//	$this->objMail->addBCC('recyclepro.co.uk+eeb5f46e46@invite.trustpilot.com');
	...
	$update_sql_array['shipping_status'] = $shipping_status;
	$update_sql_array['payment_status']=$payment_status;
	$update_sql_array['update_time']=$dte;
	$this->db->pdoupdate(WR_ORDER,$update_sql_array,'id',$order_id);
